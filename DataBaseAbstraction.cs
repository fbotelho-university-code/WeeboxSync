...
public class DataBaseAbstraction {
    ...
    public Bundle GetBundle(string id)
    {
        ...
        try
        {
           ...
           query = new SqlCommand(string.Format ("SELECT * FROM ficheiro where bundleID = '{0}'", id), con);
            ...
            reader.Close();
            query = new SqlCommand(string.Format ("SELECT * FROM tag where bundleID = '{0}'", id), con);
            reader = query.ExecuteReader();
            while (reader.Read())
            {
    ...
    public void SaveBundle(Bundle bundle)
    {
        ...
        try
        {
            ...
            {// insert new tags
                query = new SqlCommand(string.Format ("INSERT INTO tags (BundleID, tag) VALUES ('{0}', '{1}')", bundle.localId, tag), con);
                query.ExecuteNonQuery();
            }
            con.Close();
        }
    ...
    /**
        ...
        * ::Checked::
        */
    public IList<Ficheiro> GetFicheirosIDS(String bundleId) {
        ...
        try
        {
            ...
            while (reader.Read())
            {
                file = new Ficheiro(reader.GetString(2), reader.GetString(1), reader.GetString(1));
                lista.Add(file);
            }
            reader.Close();
        }
